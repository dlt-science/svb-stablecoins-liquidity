...
tick_query = """query get_ticks($date: Int = 10, $pool: String = "", $skip: Int = 10) {
  tickHourDatas(
    where: {pool: $pool, liquidityNet_not: "0", date: $date}
    skip: $skip
    orderBy: date
    orderDirection: asc
    first: 1000
...
try:
    ...
    if len(response['poolHourDatas']) == 0:
        print("pool not found")
        exit(-1)

    pool = response['poolHourDatas'][0]
    current_tick = int(pool["tick"])
    tick_spacing = fee_tier_to_tick_spacing(int(pool['pool']["feeTier"]))
...
try:
    while True:
        ...
        variables = {"skip": NUM_SKIP, "pool": POOL_ID, "date": DATE_VAL}
        response = client.execute(gql(tick_query), variable_values=variables)

        if len(response["tickHourDatas"]) == 0:
            break
        NUM_SKIP += len(response["tickHourDatas"])
        for item in response["tickHourDatas"]:
            tick_mapping[int(item['tick']["tickIdx"])] = int(item["liquidityNet"])
except Exception as ex:
...
while tick <= max_tick:
    ...
    if tick < current_range_bottom_tick:
        ...
        if should_print_tick:
            ...
            ticks.append({"tick": tick, "amount0": 0, "amount1": adjusted_amount1})

            print("        {:.2f} {} locked, potentially worth {:.2f} {}".format(adjusted_amount1, token1,
                                                                                 adjusted_amount0, token0))
        ...
        ticks.append({"tick": tick, "amount0": adjusted_amount0actual, "amount1": adjusted_amount1actual})
        ...
        if should_print_tick:
            ...
            ticks.append({"tick": tick, "amount0": adjusted_amount0, "amount1": 0})

            print("        {:.2f} {} locked, potentially worth {:.2f} {}".format(adjusted_amount0, token0,
                                                                                 adjusted_amount1, token1))
...
# # Drop ticks with zero liquidity
